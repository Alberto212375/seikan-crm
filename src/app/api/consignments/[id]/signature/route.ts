// src/app/api/consignments/[id]/signature/route.ts
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!signatureDataUrl.startsWith("data:image/")) {
      return NextResponse.json({ error: "Signature invalide (image dataUrl attendue)" }, { status: 400 });
    }
    ...
    if (!c) return NextResponse.json({ error: "Dépôt-vente introuvable" }, { status: 404 });
    ...
    const nextMeta: ConsignmentMeta = {
      ...meta,
      signature: {
        signerFirstName,
        signerLastName,
        ...
        signedAt,
        signatureDataUrl,
        context: { ip: ip || undefined, userAgent: userAgent || undefined },
      },
    };
